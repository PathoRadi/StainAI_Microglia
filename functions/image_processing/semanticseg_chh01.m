function [C, scores, allScores] = semanticseg_chh01(I, net, varargin)

%change output location and folder name
%params.WriteLocation        = strtrim(char(userInput.WriteLocation(1:sn(end-1))));
%params.OutputFolderName     = strtrim(char(userInput.WriteLocation(sn(end-1)+1:sn(end)-1)));
...
sn=strfind(userInput.WriteLocation,filesep);
...
if ~iIsAuto(value)
    ...
    for i = 1:numel(names)
        if ~isvarname(names(i))
            error(message('vision:semanticseg:ClassMustBeVarName',names(i)));
        end
    end
end
...
[~,suffix,ext]=fileparts(info);

name = cellfun(@(x,y)[x y],suffix,ext,"UniformOutput",false);
...
if isequal(strlength(outputFolderName),0)
    ...
    try        
        ...
        iThrowUnableToCreateOutputFolderMessage(ME);
    end
end

%--------------------------------------------------------------------------
function outputFolderName = iCreateUniqueFoldername(writeLocation, outputFolderName)
location = fullfile(writeLocation, outputFolderName);
k = 0;
while exist(location) %#ok<EXIST>
    k = k + 1;
    outputFolderName = outputFolderName + "_" + k;
    location = fullfile(writeLocation, outputFolderName);
end
outputFolderName = char(outputFolderName);
...
while hasdata(loader)
    ...
    if isDataBatched
        ...
        L2=L;L2(L==2)=0;L2(L==1)=255;
        filenames(idx) = iWritePixelLabelData(uint8(L2), idx, params, numImages, info);
        ...
        for i = 1:numel(idx)
            ...
            L2=L;L2(L==2)=0;L2(L==1)=255;
            filenames(idx(i)) = iWritePixelLabelData(uint8(L2), idx(i), params, numImages, info(i));
...
%--------------------------------------------------------------------------
function iAssertNotBigImageDatastore(I)
if isa(I,'bigimageDatastore')
    error(message('vision:semanticseg:bigImageDatastoreNotSupported'));
end
