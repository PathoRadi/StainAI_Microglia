function cocostruct=coco_category_update_v02(cocostruct,data1,varargin)
% (Co2) cocoStructure=coco_category_update_v02(cocoStructure,data1,varargin)
%     update coco category for atlas on category_id2, category_id2_name, category_id4, category_id4_name, only for image in the size of atlas
%{   
      input:
          cocostruct.info
                    .licenses
          ...
          data1.atlas_brain
               .atlas_table
               .categories
          varargin{1} = options.case = 'atlas' % update with brain atlas 
                                               % or when brain atlas update
      output:
          cocoStructure.categories = data1.categories
          cocoStructure.annotations  <= from cocoApi, with new category
          cocoStructure.info.Brain_Area
                       .info.atlas_table
%}
...
switch options.case
    case 'atlas'
        ...
        if isfield(data1,'atlas_brain')==1;
            ...
            cocostruct.categories=data1.categories;
            bbox_center=fix([bbox(:,1)+bbox(:,3)/2 bbox(:,2)+bbox(:,4)/2]./abs(options.Low_res));%x,y
    ...
    case 'atlas_table'
        cocostruct.info.atlas_table=table2struct(data1.atlas_table)';
end